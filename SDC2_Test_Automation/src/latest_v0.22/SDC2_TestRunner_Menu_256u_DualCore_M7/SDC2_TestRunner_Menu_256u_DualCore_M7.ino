/*
  ...
  Purpose:
    Full SDC2 characterization at 256 microsteps.

  IMPORTANT HARDWARE SETTING:
    R725 = 256 microsteps
    SW1 = D, SW2 = D, SW3 = D, SW4 = U
    Pulses per motor rev = 200 * 256 = 51200
...
const double MICROSTEP_SETTING = 256.0;     // R725 set to 256 ustep
...
// Full target-side real-time rewrite + dual-core buffered acquisition roadmap:
//   - M7 remains the deterministic actuator/control target.
//   - M4 companion sketch provides the acquisition/logging worker scaffold.
//   - Current M7 sketch preserves stable live telemetry and ISR timing.
...
void printHeader() {
  Serial.println("# SDC2 Full Characterization Firmware - 256 ustep");
  Serial.println("# FirmwareVersion,SDC2_FullChar_256u_ISR_Realtime_DualCore_M7_v2");
  Serial.println("# R725_MICROSTEP_SETTING,256");
  ...
  Serial.println("# DUAL_CORE_ROLE,M7_REALTIME_CONTROL_TARGET");
  Serial.println("# DUAL_CORE_NOTE,M4 companion sketch included for buffered DAQ scaffold; validate board FQBN before upload");
  printCsvHeader();
}
...
void runDebugTest() {
  Serial.println("# Starting DEBUG test at 256 ustep");
  zeroEncoder();
