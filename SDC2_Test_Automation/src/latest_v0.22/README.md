...
## Final dual-core + microstep sweep package notes

This package includes multiple R725 microstep firmware variants for sweep testing: 2, 4, 5, 8, 10, 16, 18, 20, 32, 50, 64, 100, 128, 180, 200, and 256 microsteps. The physical R725 DIP switches must match the selected GUI firmware. See `docs/R725_microstep_step_rate_table.md`.

For Arduino GIGA dual-core upload, this package uses one base board target with menu options, not two separate board names:

```text
M7: arduino:mbed_giga:giga:split=75_25,target_core=cm7,security=none
M4: arduino:mbed_giga:giga:split=75_25,target_core=cm4,security=none
```

Use the GUI button `Compile/upload M4 + selected M7` when testing the dual-core scaffold.
